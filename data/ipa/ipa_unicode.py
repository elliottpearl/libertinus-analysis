...
- marks and bases requiring anchors
...
# Bases
# Catalog of all bases for marks
...
BASE_IPA = [
    ...
    0x0290,0x0291,0x0292,0x0293,0x0294,0x0295,0x0296,0x0297,0x0299,0x029B,
    0x029C,0x029D,
    # 0x029E, 
    0x029F,
...
Modifiers and superscripts (count: 61)
    0x02B0,0x02B1,0x02B2,0x02B3,0x02B4,0x02B5,0x02B6,0x02B7,0x02B8,0x02E0,
    0x02E1,0x02E2,0x02E3,0x02E4,0x02C0,0x02C1,0x02C2,0x02C3,0x02C4,0x02C5,
    ...
    0x1D40,0x1D41,0x1D42,0x1D43,0x1D44,0x1D45,0x1D46,0x1D47,0x1D48,0x1D49,
    0x1D4A,0x1D4B,0x1D4C,0x1D4D,0x1D4E,0x1D4F,0x1D50,0x1D51,0x1D52,0x1D53,
    0x1D54,0x1D55,0x1D56,0x1D57,0x1D58,0x1D59,0x1D5A,0x1D5B,0x1D5C,0x1D5D,
    0x1D5E,0x1D5F,0x1D60,0x1D61
"""
...
# characters with a small capital alt in Libertinus
base_small_capital = [
    # a-z
    0x0061,0x0062,0x0063,0x0064,0x0065,0x0066,0x0067,0x0068,0x0069,0x006A,
    0x006B,0x006C,0x006D,0x006E,0x006F,0x0070,0x0071,0x0072,0x0073,0x0074,
...
BASE_PRECOMPOSED_ANCHOR_RELEVANT = [
    ...
    0x1EB9,0x1EC7,0x1EBF,0x1EC1,0x1EC5,0x00EC,0x00ED,0x00EE,0x0129,0x012B,
...
VOWELS = [
    ...
    0x0061,0x0065,0x0069,0x006F,0x0075,0x0079,0x00E6,0x0153,0x0250,0x0251,
    0x0252,0x025B,0x025C,0x025E,0x0259,0x0258,0x0264,0x026F,0x028A,0x028C,
...
# A-Z, a-z, CONSONANTS, VOWELS (count: 106)
BASE_HEURISTICS = [
    0x0041,0x0042,0x0043,0x0044,0x0045,0x0046,0x0047,0x0048,0x0049,0x004A,
    0x004B,0x004C,0x004D,0x004E,0x004F,0x0050,0x0051,0x0052,0x0053,0x0054,
    ...
    0x0079,0x007A,0x00E6,0x014B,0x0153,0x0250,0x0251,0x0252,0x0253,0x0255,
    0x0256,0x0257,0x0258,0x0259,0x025B,0x025C,0x025E,0x0260,0x0261,0x0262,
    0x0263,0x0264,0x0265,0x0266,0x026B,0x026C,0x026D,0x026E,0x026F,0x0271,
    0x0272,0x0273,0x0274,0x0275,0x0276,0x0278,0x0279,0x027A,0x027B,0x027D,
    0x0280,0x0281,0x0282,0x0283,0x0288,0x028A,0x028B,0x028C,0x028E,0x028F,
    0x0290,0x0292,0x029B,0x029D,0x029F,0x03B2,
]

# Marks
# Catalog of all combining marks in Unicode, with support in Libertinus
...
# Deprecated or disfavored marks, in Unicode, 
# and not used in practice in linguistics
MARK_LEGACY = [
...
MARK_RARE = [
    ...
    0x030E, # double vertical line above (UPA)
    0x0346, # bridge above (UPA)
    0x034A, # not tilde above (UPA)
    0x034B, # homothetic above (UPA)
    0x034C, # almost equal above (UPA)
    0x0350, # right arrowhead above (UPA)
    0x0351, # left half ring above (UPA)
    0x0352, # fermata above (UPA)
    0x0357, # right half ring above (UPA)
    0x035B, # zigzag above (UPA)
    # below
    0x0316, # grave below (UPA tone)
    0x0317, # acute below (UPA tone)
    0x0318, # left tack below (advanced)
    0x0319, # right tack below (retracted)
    0x031C, # left half ring below (UPA)
    0x0324, # diaeresis below (breathy voice)
    0x0326, # comma below (IPA)
    0x032B, # inverted double arch below (UPA)
    0x032D, # circumflex below (UPA)
    0x032E, # breve below (UPA)
    0x032F, # inverted breve below (tone)
    0x0331, # macron below (Africanist)
    0x0339, # right half ring below (retroflex)
    0x033C, # seagull below (labialization)
    0x0347, # equals sign below (Africanist)
    0x0348, # double vertical line below (UPA)
    0x0349, # left angle below (UPA)
    0x034D, # left-right arrow below (UPA)
...
MARK_BASE = {
    ...
    # Rare marks
    # above
    0x0305: VOWELS,                # overline (length, tone)
    0x030E: VOWELS,                # double vertical line above (UPA)
    0x0346: CONSONANTS,            # bridge above (UPA)
    0x034A: CONSONANTS,            # not tilde above (UPA)
    0x034B: CONSONANTS,            # homothetic above (UPA)
    0x034C: CONSONANTS,            # almost equal above (UPA)
    0x0350: CONSONANTS,            # right arrowhead above (UPA)
    0x0351: CONSONANTS,            # left half ring above (UPA)
    0x0352: VOWELS,                # fermata above (UPA)
    0x0357: CONSONANTS,            # right half ring above (UPA)
    0x035B: CONSONANTS,            # zigzag above (UPA)
    # below
    0x0316: VOWELS,                # grave below (UPA tone)
    0x0317: VOWELS,                # acute below (UPA tone)
    0x0318: CONSONANTS,            # left tack below (advanced)
    0x0319: CONSONANTS,            # right tack below (retracted)
    0x031C: CONSONANTS,            # left half ring below (UPA)
    0x0324: VOWELS,                # diaeresis below (breathy voice)
    0x0326: CONSONANTS,            # comma below (IPA)
    0x032B: CONSONANTS,            # inverted double arch below (UPA)
    0x032D: CONSONANTS,            # circumflex below (UPA)
    0x032E: VOWELS,                # breve below (UPA)
    ...
    0x034D: CONSONANTS,            # left-right arrow below (UPA)
...
unicode_groups = {
    ...
    "BASE_HEURISTICS": {
        "label": "Bases with semantic data for anchor positioning",
        "items": BASE_HEURISTICS,
    },
